use crate::errors::{ScriptError, StateError};
...
use governance_lib::{client::Client, proposal::Proposal, token_owner::TokenOwner};

macro_rules! println_bold {
    ($format:literal, $($item:expr),*) => {
        info!(concat!("\x1b[1m", $format, "\x1b[0m"), $($item),*)
    };
    ($format:literal) => {
        info!(concat!("\x1b[1m", $format, "\x1b[0m"))
    };
}

macro_rules! println_item {
    ($format:literal, $($item:expr),*) => {
        info!(concat!("\x1b[34m", $format, "\x1b[0m"), $($item),*)
    }
}

macro_rules! println_correct {
    ($format:literal, $($item:expr),*) => {
        info!(concat!("\x1b[32m", $format, "\x1b[0m"), $($item),*)
    }
}

macro_rules! println_update {
    ($format:literal, $($item:expr),*) => {
        info!(concat!("\x1b[33m", $format, "\x1b[0m"), $($item),*)
    }
}

macro_rules! println_error {
    ($format:literal, $($item:expr),*) => {
        error!(concat!("\x1b[31m", $format, "\x1b[0m"), $($item),*)
    }
}
        ...
        if let Some(data) = object {
            ...
            match verify(&data) {
                ...
                Ok(Some((instructions, signers))) => {
                    if self.verbose {
                        println_item!("{}: {:?}", name, instructions);
                    };
    ...
    pub fn execute_transaction(&self) -> Result<Option<Signature>, ScriptError> {
        if self.setup && !self.instructions.is_empty() {
            let result = if self.signers.is_empty() {
                self.client
                    .send_and_confirm_transaction_with_payer_only(&self.instructions)
            } else {
                self.client
                    .send_and_confirm_transaction(&self.instructions, &self.signers)
            };
            match result {
                Ok(signature) => {
...
impl<'a> ProposalTransactionInserter<'a> {
    pub fn insert_transaction_checked(
        &mut self,
        name: &str,
        ...
        for instruction in &instructions {
            extra_signers.extend(instruction.accounts.iter().filter(|a| {
        ...
        if !extra_signers.is_empty() {
            ...
            println_error!(
                ...
                name,
                self.proposal_transaction_index,
                error
            );
            if self.setup {
                return Err(error.into());
        ...
        if let Some(transaction_data) = self
            .proposal
            .get_proposal_transaction_data(0, self.proposal_transaction_index)?
        {
            if self.verbose {
                println_item!(
                    "Proposal transaction '{}'/{}: {:?}",
            ...
            println_update!(
                "Proposal transaction '{}'/{} was inserted in trx: {}",
                name,
                self.proposal_transaction_index,
                signature
            );
            ...
            println_update!(
                "Proposal transaction '{}'/{} will be inserted",
                name,
                self.proposal_transaction_index
            );
            if self.verbose {
