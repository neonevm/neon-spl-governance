use crate::{
    ...
    prelude::info,
    AccountOwner, Configuration, ExtraTokenAccount, TOKEN_MULT,
};
use solana_sdk::pubkey::Pubkey;

use governance_lib::addin_fixed_weights::{AddinFixedWeights, VoterWeight};
...
impl<'a> TokenDistribution<'a> {
    pub fn new(
        ...
        fixed_weight_addin: &AddinFixedWeights,
    ) -> Result<Self, ScriptError> {
        let params = fixed_weight_addin.get_params()?;
        let unlocked_amount = params
            .get("PARAM_EXTRA_TOKENS")
            .ok_or(StateError::InvalidVoterList)?
            .parse::<u64>()
            .unwrap();
        ...
        info!(
            "Vesting: {}.{:09}, unlocked: {}.{:09}, total: {}.{:09}",
            vesting_amount / TOKEN_MULT,
            vesting_amount % TOKEN_MULT,
            unlocked_amount / TOKEN_MULT,
            unlocked_amount % TOKEN_MULT,
            total_amount / TOKEN_MULT,
            total_amount % TOKEN_MULT
        );
    ...
    pub fn get_special_accounts(&self) -> Vec<Pubkey> {
        ...
        unique_owners
            .iter()
            .map(|v| self.configuration.get_owner_address(v).unwrap())
            .collect()
    }
    ...
    pub fn validate(&self) -> Result<(), ScriptError> {
        ...
        if self.info.unlocked_amount != unlocked_amount {
            info!(
                " unlocked_amount {} doesn't equal sum of amounts in extra_token_accounts {}",
        ...
        for owner in self.get_unique_owners().iter() {
            let locked_amount = self
                ...
                .iter()
                .filter_map(|v| {
                    if v.lockup.is_locked() && v.owner == *owner {
                        Some(v.amount)
                    } else {
                        None
                    }
            ...
            if locked_amount > 0 {
                if let Some(voter_item) = voter_item {
                    if voter_item.weight == locked_amount {
                        ...
                        info!("invalid amount {}", voter_item.weight);
                        result = false;
                    }
                } else {
                    info!("missed in voter_list");
                    result = false;
                };
